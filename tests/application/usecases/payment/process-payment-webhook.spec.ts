...
describe('ProcessPaymentWebhook UseCase', () => {
  ...
  const ordersGateway = mock<OrdersGateway>()
  const sut = new ProcessPaymentWebhook(paymentRepository, paymentGateway, ordersGateway)
  ...
  it('should throw if payment is not found', async () => {
    ...
    await expect(sut.execute({ gatewayResourceId })).rejects.toThrow(NotFoundError)
  ...
  it('should handle null payment repository response', async () => {
    ...
    await expect(sut.execute({ gatewayResourceId })).rejects.toThrow(new NotFoundError('Payment not found'))
  })
  ...
  it('should set order as paid if payment is paid', async () => {
    const payment = Payment.create(orderId, 100)
    paymentGateway.getPaymentDetails.mockResolvedValue({
      orderId,
      paymentStatus: PaymentStatus.PAID
    ...
    expect(ordersGateway.setOrderAsPaid).toHaveBeenCalledWith(orderId)
  })
})
